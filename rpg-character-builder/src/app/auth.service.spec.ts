...
describe('AuthService', () => {
  ...
  it('should set cookie and authState to true on successful signin', () => { const result = service.signin('wizardlywand@hogwarts.com', 'Alohomora123!');
  expect(result).toBeTrue();
    expect(service.getAuthState().subscribe((state) => expect(state).toBeTrue()));
  expect(cookieServiceSpy.set.calls.count()).toBe(1);
  });

  it('should not set cookie and authState to truth on unsuccessful signin', () => { const result = service.signin('wrongemail@hogwarts.com', 'wrongpassword');
